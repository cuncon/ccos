...
static void idt_set_gate(uint8_t num, uint32_t base, uint16_t seg, uint8_t flag)
{
...
{
	idt_p.limit = sizeof(idt_entry_t) * MAX_IDT_ENTRY - 1;
	idt_p.base = (uint32_t)&idt;

	memset(idt, 0, sizeof(idt_entry_t) * MAX_IDT_ENTRY);
